...
# Ищем последний изменённый markdown-файл в src/content/blog/
# (путь адаптирован под структуру этого проекта — Astro рендерит только src/content/blog)
...
# Проверка: обложка (строка "heroImage:"/"cover:"/"image:" в frontmatter)
if ! grep -qE "^(heroImage|cover|image):" "$ARTICLE" 2>/dev/null; then
